...
function parseData(input) {
    ...
    for (const key in input) {
        ...
        const rawVal = input[key].value;
        let y;

        try {
            y = parseInt(rawVal, base);
            if (isNaN(y)) throw "Invalid base value";
            points.push([x, y]);
        } catch {
...
function findSecret(inputJson, prime = 9999991) {
    const { points, k } = parseData(inputJson);
    ...
    console.log("Wrong/Invalid Key-Value Pairs:");
    wrongValues.forEach(([x, y]) => console.log(`(${x}: ${y})`));

    console.log("\nFinal Recovered Secret (f(0)):");
